    ...
    while(i<6)
    {
        ...
        case 3:
            ...
            for (i = 0; i < 3; i++) {
                ...
                printf("\nSubject3 marks:");
                scanf("%d",&stdArr[i].s3);
                if(stdArr[i].s1<35||stdArr[i].s2<35||stdArr[i].s3<35)
                stdArr[i].gd='F';
                stdArr[i].tot=stdArr[i].s1+stdArr[i].s2+stdArr[i].s3;
                stdArr[i].avg=stdArr[i].tot/3;
            }
            for (i = 0; i < 3; i++) {
                printf("\nStudent #%d Detail:\n", (i+1));
